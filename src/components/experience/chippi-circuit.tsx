...
const ChippiMark = () => (
  ...
  <img src="/favicon.png" alt="Helix" width={512} height={512} className="h-7 w-7 rounded-md" />
);

const NODES = [
  // Inputs — where business actually arrives from.
  { id: "lead", x: 90, y: 70, label: "New request", icon: <MessageSquare className="h-4 w-4" />, status: "active" as const },
  ...
  // The core.
  { id: "chippi", x: 330, y: 180, label: "Helix", icon: <ChippiMark />, status: "processing" as const, size: "lg" as const },
  // What it does — finished work, not proposals.
  { id: "draft", x: 580, y: 70, label: "Reply drafted, house style", icon: <PenLine className="h-4 w-4" />, status: "active" as const },
  { id: "tour", x: 580, y: 180, label: "Review booked", icon: <CalendarCheck className="h-4 w-4" />, status: "active" as const },
  { id: "deal", x: 580, y: 290, label: "Build advanced", icon: <KanbanSquare className="h-4 w-4" />, status: "active" as const },
];

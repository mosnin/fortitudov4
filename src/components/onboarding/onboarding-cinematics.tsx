 ...
 *                               holds, swaps to the Fortitudo mark, then the
 ...
 *
 * Both share the same motion vocabulary: `motion/react`, the Apple-ish expo
 * ease, the blur-rise transition, the serif title font, theme-aware bg/fg,
 * and `prefers-reduced-motion` → render instantly.
 *
...
const INTRO_LINE = 'Introducing the future of product development.';
          ...
          <AnimatePresence mode="wait">
            {stage === 'line' ? (
              ...
              <motion.div
                ...
                {...rise}
              >
                <BrandLogo className="h-8 sm:h-9" alt="Fortitudo" textClassName="text-xl" />
              </motion.div>
            )}
          </AnimatePresence>
        </motion.div>
...
export function OnboardingReady({
  ...
  onDone: () => void;
  /** The working-words that flash one at a time. */
  words?: readonly string[];
  /** The line that lands before the dissolve. */
  finalLine?: string;
}) {

...
import { Faq } from "./faq";
import { servicePdf, serviceOffer, serviceEnquiry, type ServiceCatalogEntry } from "@/lib/service-catalog";
...
export function ServiceDetail({ service: s }: { service: ServiceCatalogEntry }) {
  ...
  return <>
    ...
    <section className="pb-20 sm:pb-28"><div className={wrap}>
      <nav aria-label="On this service page" className="mb-10 flex flex-wrap gap-x-7 gap-y-1 border-y border-border py-3 text-sm text-muted-foreground">{[["The offer","offer"],["What we deliver","scope"],["Delivery","delivery"],["Service pitch","pitch"]].map(([label,id])=><a key={id} href={`#${id}`} className="inline-flex min-h-11 items-center hover:text-foreground focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-ring">{label} ↓</a>)}</nav>
      <div id="offer" className="grid scroll-mt-28 gap-10 lg:grid-cols-[1.05fr_1fr] lg:gap-20">
        <div className="py-2"><p className="text-sm text-muted-foreground">The engagement</p><h2 className="mt-4 text-4xl leading-tight tracking-tight sm:text-5xl">{offer.name}</h2><p className="mt-6 max-w-xl text-xl leading-8">{offer.summary}</p><p className="mt-5 max-w-xl text-base leading-7 text-muted-foreground">{s.audience}</p><div className="mt-8 flex flex-wrap items-center gap-6"><ArrowButton href={serviceEnquiry(s)} className="">{s.cta}</ArrowButton><a href={servicePdf(s)} download className="inline-flex min-h-11 items-center text-sm underline underline-offset-4">Download the pitch ↓</a></div><p className="mt-6 max-w-lg text-sm leading-6 text-muted-foreground">{offer.firstStep}</p></div>
        <OfferDeliverables items={offer.deliverables} />
      </div>
    </div></section>
    ...
    <section id="pitch" className="scroll-mt-28 pb-20 sm:pb-28"><div className={wrap}><div className="grid items-center gap-8 border-y border-border py-8 sm:py-10 md:grid-cols-[240px_1fr] lg:grid-cols-[280px_1fr_auto]"><Link href={`/resources/${s.resourceSlug}`} className="relative block aspect-[16/9] w-full overflow-hidden rounded-lg bg-muted md:w-full"><Image src={servicePhoto(s.slug)} alt={`${s.name} editorial photograph`} fill sizes="(min-width:768px) 280px, 100vw" className="object-cover"/></Link><div><p className="text-sm text-muted-foreground">Service deck · PDF</p><h2 className="mt-3 text-3xl tracking-tight">Take the details to your team.</h2><p className="mt-4 max-w-xl text-[15px] leading-7 text-muted-foreground">The scope, deliverables, delivery and handover in one document. Read it here or download it without submitting your email.</p><Link href={`/resources/${s.resourceSlug}`} className="mt-4 inline-flex min-h-11 items-center text-sm underline underline-offset-4">Read the pitch →</Link></div><ArrowButton href={servicePdf(s)} download className="">Download PDF</ArrowButton></div></div></section>

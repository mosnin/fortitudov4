...
const LINKS = [["Services", "/services"], ["Work", "/work"], ["About", "/about"], ["Journal", "/blog"], ["Contact", "/contact"]];
...
export function MotionShell({ children, controls }: { children: ReactNode; controls?: ReactNode }) {
  ...
  return <div ref={root} data-drawer-navigation data-open={open ? "" : undefined} data-active={active ? "" : undefined}>
    <header className="premium-header" aria-label="Site"><Logo className="h-9" /><ArrowButton href="/contact" className="header-cta">Get a proposal</ArrowButton></header>
    ...
    <nav ref={drawer} className="drawer" id="navigation-06-menu" aria-label="Main navigation" aria-hidden={!open} inert={!open} data-lenis-prevent>
      <div className="drawer-content"><p className="eyebrow">Fortitudo</p><ul className="links">{LINKS.map(([label, href]) => <li key={href}><Link href={href} aria-current={pathname === href ? "page" : undefined}><span>{label}</span></Link></li>)}</ul><div className="divider" aria-hidden="true" /><div className="socials"><Link href="/pricing"><span>Pricing</span></Link><Link href="/resources"><span>Resources</span></Link><Link href="/sign-in"><span>Client sign in</span></Link><a href="mailto:hello@fortitudo.agency"><span>hello@fortitudo.agency</span></a></div></div>
    </nav>

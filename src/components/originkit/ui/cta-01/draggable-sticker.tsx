...
//
// The peel/drag engine, kept as delivered. Only two things have been changed,
// and both are noted where they happen:
//
//  1. React's `MouseEvent` / `TouchEvent` are imported UNDER ALIASES. They used
//     to be imported under their own names, which shadowed the DOM types of the
//     same name for the whole module — so the two `window.addEventListener`
//     calls were handed listeners typed as React synthetic events and the file
//     did not type-check. The React types are what the JSX handlers want and
//     the DOM types are what `window` wants; aliasing lets both be said.
//  2. A `peel` flag, for `prefers-reduced-motion`. It suppresses the lift/peel
//     ANIMATION only — the note still drags, because dragging is the visitor
//     moving something, not the page moving on its own.
//
// Everything else — pointer handling, the incrementing z-index that lifts the
// note being dragged above its siblings, the velocity tilt, the settle, the
// WebGL mesh deformation — is untouched.
...
interface StickerDragProps {
  ...
  /**
   * Whether grabbing the sticker plays the peel. Pass `false` under
   * `prefers-reduced-motion`: the note still drags — that is the visitor moving
   * it, and taking it away would remove the interaction rather than the motion
   * — but the paper does not lift, curl, tilt or animate its shadow, so nothing
   * on screen moves except the thing under the pointer.
   */
  peel?: boolean;
  style?: CSSProperties;
}
...
// Tweakables from original. `DRAG_MAX_TILT_DEG` and `SHEEN_STRENGTH` came
// across as defaults for controls the Framer property panel used to set; both
// values now arrive as props, and neither constant had a reader.
 ...
 */
/* Shadows are the absence of light rather than a colour in the palette, so
   these stay neutral black at low alpha; the caller overrides both. */
...
/* The 1–10 `elevation` prop maps onto this internal ceiling. The two
   `map*DisplayToInternal` helpers that used to sit here, and the three display
   defaults beside them, drove the Framer property panel; nothing in this file
   or any caller read them. */
...
export default function StickerDrag(__props: StickerDragProps) {
  ...
  /* The tick schedules its own next frame, which a `useCallback` cannot do by
     name — the identifier is not in scope inside its own initialiser, and the
     lint rule that catches this is right: the captured `tick` would be the one
     from the render that started the loop, so a later `draw` or `sheenMode`
     would never reach it. The frame is requested through a ref that an effect
     keeps pointed at the current tick, so a loop started five renders ago
     still runs today's code. */
  ...
  const tick = useCallback(
    (timestamp: number) => {
      ...
      if (state.held || state.peeling || state.sticking || holoDecayActive) {
        animationRef.current = requestAnimationFrame((next) =>
  ...
  // Load image
  useEffect(() => {
    /* Read once, here, rather than in the cleanup. `stateRef` holds a plain
       mutable object that is created with the component and never reassigned,
       so the two are the same object either way — but the lint rule cannot
       know that, and the pattern it is warning about (a ref to a DOM node,
       replaced by the time cleanup runs) is a real bug elsewhere in this file's
       family of effects. */
    const state = stateRef.current;
  ...
  const handleMouseDown = useCallback(
    (e: ReactMouseEvent) => {
      ...
      state.held = true;
      // Reduced motion: held but not peeling. The note follows the pointer;
      // the paper never leaves the board.
      state.peeling = peel;
      state.sticking = false;
    ...
    [ensureTickRunning, peel],
  );

  // Mouse move handler
  useEffect(() => {
    // The DOM `MouseEvent`, not React's — this one goes to `window`.
  ...
  useEffect(() => {
    // The DOM `TouchEvent`, not React's — this one goes to `window`.

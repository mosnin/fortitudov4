...
// OriginKit footer-02 `tetris.tsx`, vendored verbatim — the decorative
// tetromino stack that plays itself along the footer card's bottom edge.
// The host (giga/footer.tsx) passes the site's colours in via props.
...
function OriginkitBaseTetris(props: TetrisProps) {
  ...
  useEffect(() => {
    ...
    function loop(time: number) {
      if (!alive) return;
      if (!running()) {
        draw(); // leave a settled frame, not a blank strip
        return; // wake() reschedules on return to view
      }
    ...
    // ADAPTED (site frame-budget rule): this is a decorative loop, and the
    // footer it lives in is pinned under the page — off-screen for almost
    // the whole visit. The loop runs only while the canvas is actually in
    // view AND the tab is visible; `last` resets on resume so the pause
    // never arrives as one huge dt. Under prefers-reduced-motion the board
    // draws its opening (startFilled) frame once and never animates.
    const reduceMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    let inView = true;
    let pageVisible = !document.hidden;
    const running = () => alive && !reduceMotion && inView && pageVisible;
    const wake = () => {
      if (!running()) return;
    ...
    if (reduceMotion) {
      draw(); // the startFilled board, once — stillness for those who asked
    } else {
      raf = requestAnimationFrame(loop);
    }

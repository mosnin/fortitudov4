...
// @ts-nocheck
/* Vendored engine. Its internals are typed `any` throughout and it caches a
   render-target check in a ref during render; both are how it shipped, and the
   file is kept as delivered so it can be re-vendored. The two rules are turned
   off HERE, in the file that breaks them, rather than widened in
   eslint.config.mjs — nothing else in the tree gets the exemption. */
/* eslint-disable @typescript-eslint/no-explicit-any */
/* eslint-disable react-hooks/refs */
"use client";
// Particle Sphere — Originkit
// Originkit — defaults rewritten to match preview.
// Vendored unchanged apart from this header and ONE added prop: the file drives
// a WebGL canvas from effects, so it has to be a client component, and its
// internal refs are typed `any` throughout, which strict TS will not take.
//
// This is the engine behind `marketing/giga/global-reach.tsx` — the only thing
// left of the hero-33 drop. Its Figma-skinned wrappers (section-hero, sphere,
// grid-backdrop, stats) were deleted: they carried a blue/#edeff3 palette, a
// serif-adjacent `font-tight` face and invented figures, none of which survive
// the port. The near-identical `particlesphere.tsx` was deleted too — nothing
// imported it.
//
// THE ADDED PROP — `autoRotate`. `prefers-reduced-motion` has to stop the
// unprompted spin without deleting the globe, and the drop offered no way to do
// that: `speed` is remapped through `mapSpeedUiToInternal`, which never reaches
// zero for a sane input and also scales the cursor physics on its way past. So
// rotation gets its own switch. With `autoRotate={false}` the engine renders the
// full lit sphere, settles, and lets its rAF loop stop — and a drag or a cursor
// still wakes it, because those are motion the visitor asked for.
...
interface ParticleSphereRefactorProps {
  ...
  /** Unprompted spin. `false` freezes it; drag and cursor still work. */
  autoRotate?: boolean;
  dragSpeed?: number;
  drag?: boolean;
  cursorOn: boolean;
...
export default function ParticleSphereRefactor(
  ...
  // Map UI speed to internal speed. `autoRotate={false}` returns a hard zero,
  // which is the value every auto-rotation branch below already tests for —
  // the loop keeps running for drag and cursor, it just stops adding spin.
  ...
  }, [speedN, rotationDirection, autoRotate]);

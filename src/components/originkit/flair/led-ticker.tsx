...
/**
 * LED dot-matrix ticker — OriginKit "Pixel LED Display", vendored from the
 * snippet supplied in-conversation (no registry fetch). Verbatim apart from
 * this note and the baked preset wrapper, which is gone: colour, copy and
 * speed are the CALLER's business (`marketing-flair.tsx`), the same rule
 * every vendored canvas in this repo follows. A 5×7-glyph LED board drawn to
 * canvas and scrolled; a caller that passes `speed={0}` and `flicker={false}`
 * gets a static lit board, which is the reduced-motion rendering.
 */
...
/**
 * Value noise in 0..1, biased low so the board sits bright and dips hard now
 * and then rather than pulsing evenly. Deterministic, so the static render
 * matches.
 */
...
/**
 * Flatten items and separators into one endless strip of dot columns
 * (bit-packed rows). Word gaps and the pad around a separator are counted in
 * dot columns rather than spelled with blank glyphs, so they stay tight as
 * spread grows.
 */
function buildColumns(items: string[], separator: string, spread: number): number[] {

...
/**
 * ASCII flame — OriginKit "Ascii Flame", vendored from the snippet supplied
 * in-conversation (no registry fetch). Verbatim apart from this note, the
 * removal of the baked preset wrapper (colour and intensity belong to the
 * caller — `marketing-flair.tsx` feeds it the yellow-on-charcoal ramp), and
 * a fixed-size default container swapped for a fluid one. The classic
 * fire-propagation cellular automaton rendered as text; under
 * prefers-reduced-motion it draws one frame and never schedules another
...
const resolveCharacters = (charsetName: FireCharset | undefined | string): string =>
  (charsetName && CHARSETS[charsetName as FireCharset]) || CHARSETS[DEFAULT_CHARSET_KEY];
...
const updateParticles = (
  ...
  const updatedParticles = particles
    ...
    .filter(
      (particle) =>
        particle.life > 0 && particle.y >= 0 && particle.x >= 0 && particle.x < columns,
    );
...
const renderFire = (
  ...
  for (let row = 0; row < rows; row += 1) {
    ...
    for (let column = 0; column < columns; column += 1) {
      ...
      const color = particleGlyph?.color ?? (value < 0.025 ? '' : safePalette[paletteIndex]);
      const character =

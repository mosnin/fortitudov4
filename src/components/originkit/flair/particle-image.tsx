/* eslint-disable @typescript-eslint/ban-ts-comment, react-hooks/refs --
   vendored Framer-era physics; see the docblock for why it stays untyped.
   The render-time ref writes are upstream's own prop-snapshot pattern — the
   render loop reads them, React never does. */
// @ts-nocheck
'use client';

/**
 * SVG/image particle field — OriginKit "SVG Particle", vendored from the
 * snippet supplied in-conversation (no registry fetch). Verbatim apart from
 * this note and two removals: the baked preset wrapper (the caller passes its
 * own config — `/about` feeds it the brand mark from `public/brand/`, which
 * is same-origin so `getImageData` never taints), and the third-party
 * `imagedelivery.net` fallback image, because a decorative canvas must not
 * phone another company's CDN when a caller forgets a prop — it renders
 * nothing instead. `@ts-nocheck` is upstream's, kept: typing seven hundred
 * lines of Framer-flavoured physics buys nothing we test.
 *
 * It samples the image's pixels once, then draws each sampled pixel as a
 * particle that returns home, roams, or shies away from the cursor.
 */
...
export default function ParticleImage(__props) {
  ...
  const { repulsionForce = 6, repulsionRadius = 80, repulsionMode = 'outside' } =
    repulsionConfig || {};
  const {
    image: rawImage,
    mode = 'fill',

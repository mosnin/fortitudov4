...
export interface KeyboardProps {
  ...
  /**
   * LOCAL ADDITION to the vendored component (everything else here is the
   * upstream registry file, MIT, from sahilcodex.vercel.app/r/keyboard.json).
   *
   * `KEYCODE`s to draw as called-out, without pressing them. The upstream
   * component only lights a key from a real press — physical or pointer — so
   * it can illustrate itself but cannot illustrate anything else. This is the
   * prop that lets a caller point at keys: hover a shortcut, see where it
   * lives. Without it the board is decoration, which this product's design
   * system does not allow.
   *
   * Highlight is drawn separately from `pressed`: a highlighted key keeps its
   * full height, so pointing at a key never looks like holding it down.
   */
...
interface KeyboardContextType {
  ...
  /** LOCAL ADDITION — see `KeyboardProps.highlightedKeys`. */
...
function KeyboardProvider({
  ...
  // LOCAL ADDITION — see `KeyboardProps.highlightedKeys`. Keyed on the joined
  // list rather than the array identity so a caller passing a fresh literal
  // every render (the normal way to write it) does not rebuild the Set and
  // re-render all 79 keys on every parent render.
  const highlightedKey = highlightedKeys?.join(",") ?? "";
  const highlightedSet = useMemo<ReadonlySet<string>>(
    () => new Set(highlightedKey ? highlightedKey.split(",") : []),
    [highlightedKey],
  );
...
function Key({
  ...
  const { themeName, pressedKeys, highlightedKeys, pressKey, releaseKey, triggerPointerHaptic } =
    useKeyboardContext();
    ...
    >
      <div
        className={cn(
          ...
          isPressed && "h-[45px]",
          // LOCAL ADDITION — a ring, not a press: the cap keeps its height, so
          // "this is the key" never reads as "this key is held down". `ring`
          // sits outside the cap, so it is legible against both the light and
          // the dark keycaps without touching either theme's colours.
          // White, not `ring-foreground`: the ring is drawn on the board's own
          // near-black case, which does not change with the app theme. A
          // token-coloured ring would be invisible on the light theme, which
          // is the product's default.
          isHighlighted && !isPressed && "ring-2 ring-offset-1 ring-white ring-offset-transparent",
        )}
        style={{
          width: `${width}px`,

...
export function SubHero({
  ...
  variant = 'deals',
  mockupSrc,
  mockupAlt = 'The Fortitudo dashboard',
}: SubHeroProps) {
...
const NAV: { icon: React.ElementType; label: string; badge?: string }[] = [
  { icon: MessageCircle, label: 'Helix' },
  { icon: Users, label: 'People', badge: '3 new' },
  { icon: Briefcase, label: 'Builds' },
...
const VARIANT_META: Record<DashVariant, { page: string; active: string }> = {
  deals: { page: 'Builds', active: 'Builds' },
  inbox: { page: 'People', active: 'People' },
  floor: { page: 'Studio', active: 'Builds' },
  chippi: { page: 'Helix', active: 'Helix' },
...
function PanelHead({ title, cta = 'Tell Helix' }: { title: string; cta?: string }) {
  return (
...
function DealsPanel() {
  const COLUMNS: { name: string; count: number; deals: { t: string; a: string; v: string; g: string; h: string; n: string }[] }[] = [
    { name: 'New', count: 3, deals: [{ t: 'Maison Noir', a: 'Marketing site', v: '$6,500', g: 'Fixed quote', h: 'ok', n: 'Send kickoff times' }] },
    { name: 'Scoping', count: 2, deals: [{ t: 'Atlas Ops', a: 'Booking funnel', v: '$8,800', g: 'Quote drafted', h: 'stuck', n: 'Confirm scope' }] },
    { name: 'In build', count: 4, deals: [
      { t: 'DataPulse', a: 'SaaS MVP', v: '$12,000', g: 'Fixed quote', h: 'ok', n: 'Sat 2:00 review' },
      { t: 'HelpStream', a: 'Support portal', v: '$4,200', g: 'Fixed quote', h: 'risk', n: '3 days quiet' },
    ] },
    { name: 'In QA', count: 2, deals: [{ t: 'Verde Botanica', a: 'Ecommerce build', v: '$7,350', g: 'Fixed quote', h: 'ok', n: 'Fixes shipped' }] },
  ];
  return (
    <>
...
function InboxPanel() {
  ...
  const LEADS = [
    { in: 'DM', n: 'Daniel Mercer', t: 'Hot', m: 'Can we still launch this weekend?', b: '$6.5k' },
    { in: 'PR', n: 'Priya Raman', t: 'Warm', m: 'Asking about hosting options', b: '$4.2k' },
    { in: 'JL', n: 'Jade Lin', t: 'Hot', m: 'Loved the concept, wants two more', b: '$12k' },
    { in: 'TB', n: 'The Bauers', t: 'Warm', m: 'Quote looks good, reviewing', b: '$7.4k' },
    { in: 'ZL', n: 'Portal lead', t: 'Cold', m: 'Opened the preview three times', b: '$5.1k' },
  ];
  return (
    <>
...
function IntegrationsPanel() {
  const APPS = [
    ...
    { icon: Database, n: 'HubSpot', d: 'contacts + projects, two-way' },
    { icon: FileSignature, n: 'DocuSign', d: 'send for signature' },
    { icon: MessageSquare, n: 'Slack', d: 'updates + approvals' },

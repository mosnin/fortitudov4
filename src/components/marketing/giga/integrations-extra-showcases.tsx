...
const SETUP_STEPS: ShowcaseStep[] = [
        ...
        >
          <span className="flex items-center gap-2.5">
            ...
            <span className="min-w-0">
              <span className="block text-[13px] font-medium text-white">Helix wants to access Gmail</span>
  ...
  },
  {
    ...
    desc: 'Helix starts working inside your tools right away, reading what comes in and drafting what goes out, in the house style.',
    mockup: (
      <Frost title="Live" badge="Active">
        <StatusLine tone="bg-emerald-400" label="Reading new mail in Gmail" meta="sorted against your builds" />
        <StatusLine tone="bg-emerald-400" label="Drafting replies in the house style" meta="the moment a request lands" />
        <StatusLine tone="bg-sky-400" label="Watching Google Calendar" meta="availability ready for reviews" />
      </Frost>
    ),
  },
];
  ...
  return (
    <FeatureShowcase
      ...
      headline={
        <>
          Connect once.
          <br className="hidden sm:block" /> Helix takes it from there.
        </>
      }
      product={{
        name: 'Quick Connect',
        icon: Plug,
        desc: 'Connect the tools you already pay for with a click through Composio. No engineering, no spreadsheets. Helix starts working inside them right away.',
...
const AUTOMATION_STEPS: ShowcaseStep[] = [
  {
    key: 'trigger',
    title: 'A new lead in HubSpot',
    desc: 'Helix watches your connected tools. The moment a new project lead lands in HubSpot, the recipe fires.',
    mockup: (
  ...
  {
    key: 'draft',
    title: 'Helix drafts the reply',
    desc: 'It reads the lead and drafts a first reply in the house style from your own Gmail.',
    mockup: (
      <Frost title="Recipe">
        <RecipeCard
          kind="WHEN"
        ...
        <RecipeCard
          ...
          title="Draft reply in the house style"
  ...
  },
  {
    ...
    desc: 'It finds a real open slot and sends the scoping-call invite, all against your actual availability.',
    mockup: (
      <Frost title="Recipe">
        <RecipeCard
          kind="THEN"
  ...
  },
  {
    ...
    desc: 'Every step is written back, so the project moves forward and your CRM stays the source of truth without any manual entry.',
    mockup: (
      <Frost title="Recipe">
        <RecipeCard
          kind="THEN"
          badge={<AppBadge icon={Database} name="HubSpot" />}
          title="Update the project, log every step"
          right={
            <span className="inline-flex items-center gap-1 rounded-md bg-emerald-400/15 px-2 py-1 text-[10.5px] font-medium text-emerald-300">
        ...
        />
        <Connector />
        <StatusLine tone="bg-emerald-400" label="Project stage advanced" meta="reply sent, call booked, logged" />
      </Frost>
    ),
  },
];
    ...
    <FeatureShowcase
      ...
      headline={
        <>
          ...
          <br className="hidden sm:block" /> Helix acts.
        </>
      }
      product={{
        name: 'Always On',
        icon: Workflow,
        desc: 'Helix watches your connected tools and acts on what matters, drafting, booking, and logging across them, around the clock.',
        cta: { label: 'Browse integrations', href: '/services' },
      }}

...
`design.md` is the canonical design document. The public website and auth shell use the purchased Imageworks source in `src/components/imageworks/`, with Fortitudo branding, Geist Sans and a dark default. Preserve the original theme composition, photography, spacing, navigation, scroll scenes and motion. Do not replace it with another kit. Public color tokens are scoped in `src/app/(marketing)/imageworks.css`.

Content is grounded in the 14 Notion offers captured in `docs/imageworks/notion-offers-2026-09-21.json`, represented by `src/content/service-catalog.json` and `service-offers.json`. No invented testimonials, benchmark metrics or prices. Original theme photos are visual assets, not case-study proof. Actual work screenshots belong on work pages.

The authenticated product keeps its CRM system (`src/components/crm/`, `src/components/shell/`, `src/lib/typography.ts`). Marketing changes do not authorize database or permissions changes. The former Shader/Giga/OriginKit public components and unreferenced assets were retired; do not restore them as a visual fallback.
